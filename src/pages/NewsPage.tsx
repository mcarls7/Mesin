...
const NewsPage = () => {
  ...
  const articleSchema = {
    ...
    itemListElement: NEWS.map((news, index) => ({
      '@type': 'ListItem',
      position: index + 1,
      url: `https://sealonline.co.id/news#news-${news.id}`,
      name: news.title,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          {NEWS.slice(0, 1).map((news) => (
            <FadeIn key={news.id} delay={0.1}>
              <div className="glass rounded-2xl overflow-hidden hover:border-gold/30 transition-all group">
                ...
                <div className="p-6">
                  ...
                  <div className="mt-4">
                    <ShareButtons title={news.title} url={`https://sealonline.co.id/news#news-${news.id}`} />
                  </div>
                </div>
              </div>
            </FadeIn>
